export const PRIMARY = '#1779ba'
export const SECONDARY = '#767676'
export const WHITE = '#FFFFFF'
export const BLACK = '#000000'

// ACTIONS
export const SUCCESS = '#3adb76'
export const WARNING = '#ffae00'
export const ALERT = '#cc4b37'
// BLUE
export const BLUE_DARK = '#1c74bb'
export const BLUE_MEDIUM = '#3792d0' // mau button
export const BLUE_LIGHT = '#4297d3' // vi du tieng anh
export const BLUE_TITLE = '#003a68'
export const BLUE_TEXT = '#044470' // tu dang tim kiem
export const BLUE_EXPLAN = '#01416e' // Giai thich, nghia
export const BLUE_TRANSLATE = '#8b8b8b' // vi du tieng viet

// GREEN
export const GREEN_MEDIUM = '#00c100'
export const GREEN_LIGHT = '#80ff80'
